...
function frontAgain(string) {
	if (string.substr(0, 2) === string.substr(string.length - 2, string.length)) {
		return true;
	}
	return false;
}
...
function cigarParty(numOfCigars, isWeekend) {
	if (!isWeekend && numOfCigars >= 40 && numOfCigars <= 60) {
		return true;
	}
	if (isWeekend && numOfCigars >= 40) {
		return true;
	}
	return false;
}
...
function filterBigNumbers(array) {
	const oneHundredOrMoreArray = [];
	for (let i = 0; i < array.length; i++) {
		if (array[i] >= 100) {
			const num = array[i];
			oneHundredOrMoreArray.push(num);
		}
	}
	return oneHundredOrMoreArray;
}
...
function filterMultiplesOfX(array, x) {
	...
	for (let i = 0; i < array.length; i++) {
		if (array[i] % x === 0) {
			const multipleNum = array[i];
			multiplesArray.push(multipleNum);
		}
	}
