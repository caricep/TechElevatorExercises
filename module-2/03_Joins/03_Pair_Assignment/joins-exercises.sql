...
SELECT floor(AVG(city.population)) AS average_city_population_in_europe
FROM city
JOIN country ON city.countrycode = country.code
...
SELECT floor(AVG(city.population)) AS average_city_population_in_asia
FROM city
JOIN country ON city.countrycode = country.code
...
SELECT COUNT(city.name) AS number_of_cities_where_english_is_official
FROM city
JOIN country ON city.countrycode = country.code
...
SELECT floor(AVG(city.population)) AS average_population_of_cities_where_english_is_official
FROM city
JOIN country ON city.countrycode = country.code
JOIN countrylanguage ON countrylanguage.countrycode = country.code
WHERE countrylanguage.language = 'English' AND countrylanguage.isofficial = true;
...
SELECT country.continent, MAX(city.population) AS max_population_of_cities_in_each_continent
FROM country
