...
public class JdbcPersonDao implements PersonDao {
	...
	private static final String SQL_SELECT_PERSON = "SELECT person_id, first_name, last_name, date_added FROM person";
	...
	public void save(Person person) {
		...
		Integer personId = jdbcTemplate.queryForObject(sql, Integer.class, person.getFirstName(), person.getLastName(), Date.valueOf(person.getDateAdded()));
		person.setPersonId(personId);
	}

	@Override
	public void update(Person person) {
		String sql = "UPDATE person SET first_name = ?, last_name = ?, date_added = ? WHERE person_id = ?";
		jdbcTemplate.update(sql, person.getFirstName(), person.getLastName(), Date.valueOf( person.getDateAdded()), person.getPersonId());
	}
